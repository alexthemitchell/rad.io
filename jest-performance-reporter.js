...
class PerformanceReporter {
  ...
  _printReport(results) {
    ...
    console.log(
      `  Average time per test: ${_testMetrics.averageTime.toFixed(2)}ms`,
    );
    console.log(
      `  Slow tests (>${THRESHOLDS.unit.slow}ms): ${_testMetrics.slowTests}`,
    );
    console.log(
      `  Very slow tests (>${THRESHOLDS.unit.verySlow}ms): ${_testMetrics.verySlowTests}`,
    );
    ...
    if (_slowTests.length > 0) {
      ...
      top10.forEach((test, index) => {
        const emoji =
          test.duration > test.threshold * 5
            ? "🔴"
            : test.duration > test.threshold * 2
              ? "🟡"
              : "🟢";
        console.log(
          `  ${index + 1}. ${emoji} ${test.duration.toFixed(0)}ms - ${test.name}`,
        );
      ...
      if (_testMetrics.verySlowTests > 0) {
        console.log("\n💡 Recommendations:");
        console.log(
          "  • Consider splitting very slow tests into smaller units",
        );
        console.log("  • Check for missing mocks (WebUSB, WebGL, WASM)");
        console.log(
          "  • Profile with: npm test -- --logHeapUsage --maxWorkers=1",
        );
