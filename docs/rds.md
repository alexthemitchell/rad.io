...
Metadata is keyed by channel center rather than transient track ID. It remains visible while the track is `recent`, becomes `stale` after two seconds of source time without a valid group, and is removed when the track expires. Each `AnalyzerController` forwards changed target sets to its active source and overlays low-rate source receptions onto that session's analyzer tracks by channel center.

Retuning, changing sample rate or profile, resetting, stopping, and acquisition discontinuities clear decoder state. Before an automatic HackRF retune, the source clears old targets so offsets relative to the previous center cannot reach the decoder. The acquisition owner then resets decoder synchronization and drops settling IQ. Fresh post-retune analyzer frames select the same RF channel again with a new frequency offset; metadata remains absent or `searching` until new over-the-air groups establish lock.
