...
- [x] **Cross-Browser Regression Gates (WebUSB/WebAudio)**: define a minimal browser matrix (Chrome/Edge stable + one canary) and block merges on known WebUSB/WebAudio regressions. (`docs/reference/dev/browser-regression-gates.json`, `scripts/browser-regression-gate.mjs`, `.github/workflows/e2e.yml` matrix + gating step)
...
- [x] **IQ Integrity Wizard (“Bad IQ?”)**: detect IQ swap/invert/sign/scaling, DC offset, clipping, and sample-rate mismatch; offer one-click fixes and persist results into per-device profiles. (`src/measurements/iqIntegrityWizard.ts`, `src/measurements/iqIntegrityWizard.test.ts`, `src/App.tsx` IQ wizard controls + apply/save actions, `src/devices/deviceProfileStore.ts` persisted `iqIntegrityLastReport`)
- [x] **Hardware Bring-Up & Sanity Self-Test (Real Device)**: one-click checks for sample format/IQ ordering, DC offset magnitude, clock offset, gain-step effectiveness, and stream continuity; outputs a concise pass/fail report for support bundles. (`src/measurements/hardwareSanitySelfTest.ts`, `src/measurements/hardwareSanitySelfTest.test.ts`, `src/App.tsx` `Run Hardware Self-Test` flow + diagnostics `hardwareSelfTest`)
...
- [x] **Signal ID & Tuning Advisor (“What am I seeing?”)**: heuristic hints for likely signal class (AM/FM/narrowband/digital-ish/bursty), recommended demod + bandwidth, and warnings for common false signals (images, aliasing, DC spur, LO leakage). (`src/measurements/signalIdTuningAdvisor.ts`, `src/measurements/signalIdTuningAdvisor.test.ts`, `src/App.tsx` Health Checks + diagnostics `signalIdAdvisor`)
...
- [x] **Background Audio Reliability Strategy**: implement PWA keep-alive strategies and persistent workers to prevent audio stuttering/throttling when the tab is backgrounded or the screen is off. (`src/App.tsx` wake-lock + hidden keep-alive ping guard, `src/dsp/worker.ts` keep-alive command handling)
...
- [x] **Sample-Rate Mismatch Strategy**: define how device IQ rates, DSP rates, and OS output rates interact; ensure stable behavior when output is forced to 48 kHz. (`src/measurements/sampleRateMismatchStrategy.ts`, `src/measurements/sampleRateMismatchStrategy.test.ts`, `src/App.tsx` health + diagnostics `sampleRateMismatch`)
- [x] **Click-Free Reconfiguration Contract**: guarantee pop/click suppression on start/stop, retune, bandwidth changes, and output device changes (bounded ramp + discontinuity events). (`src/audio/AudioSink.ts` transition ramp + stop ramp, `src/App.tsx` reconfiguration discontinuity breadcrumbs for start/stop/retune/bandwidth/output-device)
...
- [x] **Buffer Telemetry**: buffer occupancy graphs + underrun/overrun counters across USB → DSP → audio. (`src/measurements/bufferTelemetry.ts`, `src/measurements/bufferTelemetry.test.ts`, `src/App.tsx` Runtime Metrics + diagnostics `bufferTelemetry`)
...
- [x] **USB Transfer Scheduling Telemetry**: measure transfer size/cadence effects (short packets, burstiness, controller quirks) and surface recommended “streaming profiles”. (`src/devices/HackRFDevice.ts` cadence/burstiness/long-gap counters + recommendation in `streamingProfile`, `src/devices/ISDRDevice.ts` scheduling telemetry contract, `src/App.tsx` USB profile/recommendation controls)
...
- [x] **RF Impurity Telemetry**: track DC spur level, estimated image rejection/IQ imbalance, LO leakage indicators, and spur density/overload heuristics and surface them in diagnostics. (`src/telemetry/runtimeTelemetryContract.ts`, `src/telemetry/runtimeTelemetryContract.test.ts`, `src/dsp/worker.ts`, `src/App.tsx` Runtime Metrics + Health Checks)
...
- [x] **Suspend/Resume Resilience**: handle tab reload, sleep/wake, and mid-stream disconnects with clear UX and safe defaults. (`src/measurements/sessionResilience.ts`, `src/measurements/sessionResilience.test.ts`, `src/App.tsx` beforeunload/pagehide/pageshow + USB disconnect recovery)
- [x] **Audio Output Device Selection**: choose output device and handle hot-swaps without requiring a full restart. (`src/audio/AudioSink.ts`, `src/audio/AudioSink.test.ts`, `src/App.tsx` audio output selector + devicechange refresh)
...
- [x] **Adaptive Streaming Policy**: dynamically adjust USB transfer sizing / buffering / scheduling based on rate and CPU pressure, with explicit “degraded mode” behavior. (`src/measurements/usbStreamingPolicy.ts`, `src/measurements/usbStreamingPolicy.test.ts`, `src/App.tsx` periodic adaptive profile policy loop + diagnostics events)
...
- [x] **Safe Mode Boot (Crash/Perf Recovery)**: if last session ended badly (crash, runaway CPU, repeated disconnect), auto-start with minimal pipeline + no auto-connect and offer a guided restore. (`src/App.tsx` safe-mode marker persistence/boot defaults/recovery banner + restore actions)
...
- [x] **Crash-Only Pipeline Recovery**: if a Worker/AudioWorklet fails, automatically restart into a safe minimal pipeline and preserve a support-bundle breadcrumb trail. (`src/App.tsx` worker `onerror`/`onmessageerror`, audio push fault handling, safe-mode crash marker + auto-reload recovery path, diagnostics `pipelineConfig.safeMode`)
...
- [x] **USB Streaming Profile Auto-Tuner (Measured)**: automatically try a bounded set of transfer sizes/counts and recommend a stable profile based on drop rate/jitter/CPU, storing results per controller/hub. (`src/App.tsx` measured candidate trials via `runUsbStreamingAutoTuner`, `src/measurements/usbStreamingPolicy.ts` scoring contract)
- [x] **USB Trace Replay (Sim)**: replay captured USB-level behavior into simulated runs to reproduce WebUSB flakiness deterministically. (`src/fixtures/scenarios/usbTraceReplay.ts`, `src/fixtures/scenarios/usbTraceReplay.test.ts`, `src/devices/scenarioFixtures.test.ts` replay execution)
...
- [x] **USB Chaos / Fault Injection (Sim)**: deterministic scenarios for stalls, short packets, device resets mid-stream, and “endpoint halt storms” to harden recovery logic. (`src/fixtures/scenarios/scriptedRfScenarios.ts` USB fault events + chaos scenario, `src/devices/scenarioFixtures.test.ts` deterministic fault-run assertions)
...
- [ ] **Capability Negotiation Algorithm (Safe Defaults)**: choose rate/bandwidth/gain/streaming profile with an explainable decision trace and deterministic reapply on reconnect. (`src/devices/CapabilityNegotiationAlgorithm.ts`, `src/devices/CapabilityNegotiationAlgorithm.test.ts`) _Current state: algorithm + tests exist, but runtime connect/reconnect still applies profile settings via ad-hoc path in `src/App.tsx`._
- [ ] **Gain Stage Ordering & Validation**: device-specific gain constraints (order, ranges, coupling) with UI guidance and safe clamping. (`src/devices/GainStageValidator.ts`, `src/devices/GainStageValidator.test.ts`) _Current state: validator utilities + tests exist, but UI/device apply path does not yet use the validator as the enforcement source of truth._
...
- [x] **Bias-Tee / Antenna Power Safety**: explicit “power on” controls with warnings/timeouts where applicable. (`src/devices/ISDRDevice.ts`, `src/devices/HackRFDevice.ts`, `src/devices/HackRFDevice.test.ts`, `src/App.tsx`)
- [x] **External IO / GPIO Control**: Interface for controlling device GPIO pins (antenna switching, external filters/LNAs). (`src/devices/ISDRDevice.ts`, `src/devices/MockDevice.ts`, `src/devices/FileDevice.ts`, `src/devices/ISDRDevice.contract.test.ts`, `src/App.tsx`)
...
- [x] **LO Offset / IF Shift**: mitigate DC spike and LO artifacts. (`src/App.tsx` explicit IF-shift controls + LO-shift mitigation wiring, `src/dsp/frequencyMapping.ts`, `src/dsp/frequencyMapping.test.ts`)
...
- [x] **Amplitude Calibration Storage**: per-device/per-band calibration blobs (gain vs frequency, baseline noise estimates) to support approximate dBFS→dBm mapping with disclaimers. (`src/measurements/amplitudeCalibrationStore.ts`, `src/measurements/amplitudeCalibrationStore.test.ts`, `src/App.tsx` calibration card + diagnostics bundle)
- [x] **Level Calibration Wizard (Quasi-Absolute)**: guided workflow to map dBFS → approximate dBm/dBµV using known reference sources and RF chain context, producing an uncertainty bound that propagates into meters/exports. (`src/measurements/levelCalibrationWizard.ts`, `src/measurements/levelCalibrationWizard.test.ts`, `src/App.tsx` level calibration controls + persisted save flow)
- [x] **Band-Specific Calibration Profiles**: separate HF/VHF/UHF calibration offsets and presets (with import/export) to reduce “one knob fits all” error. (`src/measurements/amplitudeCalibrationStore.ts`, `src/measurements/amplitudeCalibrationStore.test.ts`, `src/App.tsx` band profile save + calibration bundle export)
- [x] **Clock/Timebase Model**: define how sample-clock drift and discontinuities are represented and propagated into DSP + UI. (`src/measurements/timebaseModel.ts`, `src/measurements/timebaseModel.test.ts`, `src/App.tsx` timebase model state + diagnostics payload `timebaseModel`)
- [x] **Reference Clock Support (Bring Model Forward)**: represent internal vs external 10 MHz reference (when supported), integrate into PPM/AFC semantics, and include it in diagnostics bundles. (`src/measurements/referenceClockModel.ts`, `src/measurements/referenceClockModel.test.ts`, `src/App.tsx` reference support model + diagnostics `referenceClockSupportModel`)
...
- [x] **Reference Lock “Prove It” Flow**: guided stability check (time window + confidence) so “measurement-grade” claims can require demonstrated lock/stability. (`src/measurements/referenceClockModel.ts`, `src/measurements/referenceClockModel.test.ts`, `src/App.tsx` rolling proof window + health/diagnostics `referenceLockProof`)
...
- [x] **Antenna / Front-End Context Profiles**: persist antenna name + external preamp/attenuator/filter/bias-tee notes alongside device profiles and include them in diagnostics bundles for actionable support. (`src/measurements/rfContextProfiles.ts`, `src/measurements/rfContextProfiles.test.ts`, `src/App.tsx` profile save/apply controls + diagnostics inclusion)
- [x] **RF Chain Profiles (Typed + Reusable)**: named, reusable RF chain configs (attenuator/LNA/filter/transverter/IF offsets) that can be applied per device/band and automatically reflected in measurement disclosures and tuning math. (`src/measurements/rfContextProfiles.ts`, `src/measurements/rfContextProfiles.test.ts`, `src/App.tsx` typed RF chain profile save/apply + frequency mapping apply)
- [x] **Spur/LO Artifact Catalog (Per-Device)**: maintain a per-device/per-rate/per-gain “known internal artifact” catalog to prevent false signal attribution and improve supportability. (`src/measurements/spurArtifactCatalog.ts`, `src/measurements/spurArtifactCatalog.test.ts`, `src/App.tsx` annotation capture + marker match warnings + diagnostics `spurArtifactCatalog`)
...
- [x] **Host-Assisted Hardware Sweep Path (HackRF)**: Implemented gated host-assisted `hackrf_sweep` invocation (via bridge capability) with explicit software tune/settle/stitch fallback for browser-only WebUSB sessions; pure WebUSB device-side hardware sweep remains unavailable. Residual blockers are bridge packaging/distribution outside the web app plus real-device sweep-rate validation evidence. (`src/devices/hackrfSweepHostBridge.ts`, `src/devices/hackrfSweepHostBridge.test.ts`, `src/devices/hackrfSweepFallbackPlan.ts`, `src/devices/hackrfSweepFallbackPlan.test.ts`, `src/App.tsx`, `docs/validation/phase5-3-hackrf-sweep-webusb-blockers.md`)
...
- [x] **Airspy Support**: Airspy bridge-client adapter using shared bridge protocol/auth/backpressure modules with negotiated-rate state-machine behavior and tests. Requires a separate host bridge service outside the web app for real hardware control. (`src/devices/bridge/BridgeBackedDevice.ts`, `src/devices/AirspyBridgeDevice.ts`, `src/devices/BridgeDevices.test.ts`, `src/App.tsx`)
- [x] **SDRplay (RSP) Support**: SDRplay bridge-client adapter with capability model/gain controls, token-authenticated handshake, and continuity tests. Requires a separate host bridge service outside the web app for real hardware control. (`src/devices/bridge/BridgeBackedDevice.ts`, `src/devices/SdrplayBridgeDevice.ts`, `src/devices/BridgeDevices.test.ts`, `src/App.tsx`)
- [x] **PlutoSDR / LimeSDR**: PlutoSDR + LimeSDR bridge-client adapters with source integration and contract/continuity tests. Requires a separate host bridge service outside the web app for real hardware control. (`src/devices/bridge/BridgeBackedDevice.ts`, `src/devices/PlutoSdrBridgeDevice.ts`, `src/devices/LimeSdrBridgeDevice.ts`, `src/devices/BridgeDevices.test.ts`, `src/App.tsx`)
- [x] **Bridge Transport Security Model**: local-only defaults, explicit pairing, capability discovery, authn/authz story, and redacted diagnostics capture suitable for support exports. (`src/devices/bridge/BridgeBackedDevice.ts`, `src/devices/bridge/bridgeProtocol.ts`, `src/devices/bridge/bridgeAuth.ts`, `src/devices/bridge/bridgeProtocol.test.ts`, `src/devices/bridge/bridgeAuth.test.ts`)
