    ...
    it('should produce equivalent FFT results (when WASM available)', async () => {
      ...
      if (wasmResult) {
        ...
        for (let i = 0; i < fftSize; i++) {
          expect(Math.abs((wasmResult[i] ?? 0) - (jsResult[i] ?? 0))).toBeLessThan(0.1);
        }
      } else {
    ...
    it('should produce equivalent waveform results (when WASM available)', async () => {
      ...
      if (wasmResult) {
        ...
        for (let i = 0; i < testSamples.length; i++) {
          expect(Math.abs((wasmResult.amplitude[i] ?? 0) - (jsResult.amplitude[i] ?? 0))).toBeLessThan(0.001);
          expect(Math.abs((wasmResult.phase[i] ?? 0) - (jsResult.phase[i] ?? 0))).toBeLessThan(0.001);
        }
      } else {
        expect(wasmResult).toBeNull();
      }
        ...
        for (let row = 0; row < wasmResult.length; row++) {
          ...
          if (wasmRow && jsRow) {
            ...
            for (let i = 0; i < wasmRow.length; i++) {
              expect(Math.abs((wasmRow[i] ?? 0) - (jsRow[i] ?? 0))).toBeLessThan(0.1);
            }
          }
        }
      } else {
    ...
    it('should handle samples with extreme values', async () => {
      ...
      if (wasmWaveform) {
        ...
        for (let i = 0; i < samples.length; i++) {
          const diff = Math.abs((wasmWaveform.amplitude[i] ?? 0) - (jsWaveform.amplitude[i] ?? 0));
