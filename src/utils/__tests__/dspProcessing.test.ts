...
describe("Windowing Functions", () => {
  ...
  describe("applyKaiserWindow", () => {
    ...
    it("should accept custom beta parameter", () => {
      ...
      expect(magnitude(windowed1[0]!)).not.toBeCloseTo(
        magnitude(windowed2[0]!),
        3,
      );
    });
  });
  ...
  describe("Window properties", () => {
    it("should maintain energy conservation", () => {
      ...
      const energyBefore = samples.reduce(
        (sum, s) => sum + s.I * s.I + s.Q * s.Q,
        0,
      );
      const energyAfter = windowed.reduce(
        (sum, s) => sum + s.I * s.I + s.Q * s.Q,
        0,
      );
...
describe("Automatic Gain Control (AGC)", () => {
  it("should normalize signal amplitude", () => {
    ...
    const avgMag =
      tailSamples.reduce((sum, s) => sum + magnitude(s), 0) /
      tailSamples.length;
...
describe("Decimation", () => {
  ...
  it("should preserve signal characteristics after decimation", () => {
    ...
    const avgMag =
      decimated.reduce((sum, s) => sum + magnitude(s), 0) / decimated.length;
