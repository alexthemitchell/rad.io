...
export function createMockUSBDevice(
	...
	const script = options?.transferInScript ?? buildDefaultScript(options?.transferInBehavior);
	...
	const transferIn = jest
		...
		.mockImplementation(() => {
			const step = script[Math.min(transferInCallCount, script.length - 1)] ?? "success";
