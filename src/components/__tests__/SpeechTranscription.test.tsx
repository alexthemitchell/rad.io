...
beforeAll(() => {
  ...
  (global as Window & { webkitSpeechRecognition?: typeof MockSpeechRecognition }).webkitSpeechRecognition = MockSpeechRecognition as unknown as typeof SpeechRecognition;
  ...
  // Mock SpeechSynthesisUtterance
  (global as Window & { SpeechSynthesisUtterance?: typeof MockSpeechSynthesisUtterance }).SpeechSynthesisUtterance = MockSpeechSynthesisUtterance as unknown as typeof SpeechSynthesisUtterance;
});
...
describe("SpeechTranscription Component", () => {
  ...
  describe("Rendering", () => {
    it("should render component", () => {
      ...
      expect(screen.getByLabelText(/speech transcription controls/i)).toBeInTheDocument();
    });
  ...
  describe("Mode Selection", () => {
    ...
    it("should call onModeChange when mode changes", () => {
      ...
      render(<SpeechTranscription {...defaultProps} onModeChange={onModeChange} />);
    ...
    it("should show manual info when in manual mode", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/microphone-based transcription/i)).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Language Selection", () => {
    ...
    it("should call onLanguageChange when language changes", () => {
      ...
      render(<SpeechTranscription {...defaultProps} onLanguageChange={onLanguageChange} />);
  ...
  describe("Demo Mode", () => {
    it("should use speech synthesis in demo mode", async () => {
      render(<SpeechTranscription {...defaultProps} mode="demo" signalType="FM" />);
    ...
    it("should add demo transcript", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/DEMO MODE.*FM radio broadcast/i)).toBeInTheDocument();
      });
    });

    it("should use correct phrase for signal type", async () => {
      render(<SpeechTranscription {...defaultProps} mode="demo" signalType="AM" />);
  ...
  describe("Callbacks", () => {
    it("should call onStart when recognition starts", async () => {
      ...
      render(<SpeechTranscription {...defaultProps} mode="manual" onStart={onStart} />);
    ...
    it("should call onStop when mode changes to off", async () => {
      ...
      // Change mode to off
      rerender(<SpeechTranscription {...defaultProps} mode="off" onStop={onStop} />);
  ...
  describe("Educational Notes", () => {
    it("should show educational note in manual mode", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Web Speech API uses your microphone/i)).toBeInTheDocument();
      });
    });

    it("should not show educational note in off mode", () => {
      render(<SpeechTranscription {...defaultProps} mode="off" />);
      expect(screen.queryByText(/Web Speech API uses your microphone/i)).not.toBeInTheDocument();
    });
  ...
  describe("Availability", () => {
    it("should disable mode selector when not available and mode is off", () => {
      render(<SpeechTranscription {...defaultProps} isAvailable={false} mode="off" />);
      const select = screen.getByLabelText(/mode/i);
      expect(select).toBeDisabled();
    });

    it("should not disable mode selector when mode is active", () => {
      render(<SpeechTranscription {...defaultProps} isAvailable={false} mode="demo" />);
      const select = screen.getByLabelText(/mode/i);
      expect(select).not.toBeDisabled();
    });
  });
