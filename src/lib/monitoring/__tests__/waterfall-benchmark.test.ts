...
describeIf("Waterfall Performance Benchmarks", () => {
  ...
  describe("30 FPS Target (2048 FFT)", () => {
    ...
    it("should maintain 30 FPS for 1 second", () => {
      ...
      expect(metrics.visualization.droppedFrames).toBeLessThan(
        expectedFrames * 0.01,
      ); // < 1% drops
      ...
      console.log(
        `  Dropped frames: ${metrics.visualization.droppedFrames}/${expectedFrames}`,
      );
      console.log(
...
function simulateDSPPipeline(fftSize: number): void {
  ...
  for (let i = 0; i < fftSize; i++) {
    iSamples[i] = (iSamples[i] ?? 0) - dcI;
    qSamples[i] = (qSamples[i] ?? 0) - dcQ;
  }

  // Simulate windowing (Hann)
  for (let i = 0; i < fftSize; i++) {
    const w = 0.5 * (1 - Math.cos((2 * Math.PI * i) / (fftSize - 1)));
    iSamples[i] = (iSamples[i] ?? 0) * w;
    qSamples[i] = (qSamples[i] ?? 0) * w;
  }

  // Simulate FFT (simplified - just calculate magnitudes)
  const spectrum = new Float32Array(fftSize);
  for (let i = 0; i < fftSize; i++) {
    const mag = Math.sqrt((iSamples[i] ?? 0) ** 2 + (qSamples[i] ?? 0) ** 2);
...
function simulateWaterfallRender(fftSize: number): number {
  ...
  for (let i = 0; i < fftSize; i++) {
    ...
    const idx = i * 4;
    imageData[idx] = value * 255; // R
    imageData[idx + 1] = value * 128; // G
    imageData[idx + 2] = value * 64; // B
  ...
  for (let i = 0; i < scrollBuffer.length; i++) {
    scrollBuffer[i] = imageData[i % imageData.length] ?? 0;
  }
