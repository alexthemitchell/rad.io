...
test.describe("Navigation - Page Routing", () => {
  ...
  test("should navigate to support pages", async ({ page }) => {
    const supportPages = ["/settings", "/calibration", "/help"];
  ...
  test("should highlight active page in navigation", async ({ page }) => {
    ...
    if ((await scannerLink.count()) > 0) {
      ...
      const ariaCurrentAttribute = await scannerLink
        .first()
        .getAttribute("aria-current");
      const hasActiveClass = await scannerLink
        .first()
        .evaluate(
          (el) =>
...
test.describe("Navigation - Deep Linking", () => {
  ...
  test("should preserve query parameters across navigation", async ({
    page,
  }) => {
...
test.describe("Keyboard Shortcuts - Global", () => {
  ...
  test("should support Ctrl/Cmd+S to start/stop recording", async ({
    page,
  }) => {
    await page.goto("/monitor?mockSdr=1");
  ...
  test("should support Ctrl/Cmd+F to focus frequency input", async ({
    page,
  }) => {
...
test.describe("Bookmarks Panel", () => {
  ...
  test("should support hierarchical folders", async ({ page }) => {
    ...
    const folderItems = page.locator(
      '[role="treeitem"], [aria-label*="folder" i]',
    );
  ...
  test("should support filter by tags", async ({ page }) => {
    ...
    const tagFilter = page.locator(
      '[aria-label*="tag" i], [placeholder*="tag" i]',
    );
...
test.describe("Devices Panel", () => {
  ...
  test("should allow per-device settings", async ({ page }) => {
    ...
    const deviceSettings = page.locator(
      '[aria-label*="sample rate" i], [aria-label*="gain" i]',
    );
...
test.describe("Measurements Panel", () => {
  ...
  test("should support placing markers", async ({ page }) => {
    ...
    const addMarkerBtn = page.getByRole("button", {
      name: /add.*marker|place.*marker/i,
    });
...
test.describe("Calibration Page", () => {
  ...
  test("should display calibration wizard", async ({ page }) => {
    ...
    const calibrationWizard = page.locator(
      '[aria-label*="calibration" i], .wizard',
    );
