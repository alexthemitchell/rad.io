...
test.describe("Decode - Mode Selection", () => {
  ...
  test("should support RTTY mode selection", async ({ page }) => {
    ...
    const rttyOption = page
      .getByRole("tab", { name: /rtty/i })
  ...
  test("should support PSK31 mode selection", async ({ page }) => {
    ...
    const pskOption = page
      .getByRole("tab", { name: /psk/i })
  ...
  test("should support SSTV mode selection", async ({ page }) => {
    ...
    const sstvOption = page
      .getByRole("tab", { name: /sstv/i })
...
test.describe("Decode - PSK31 Decoder", () => {
  test("should support AFC (Automatic Frequency Control)", async ({ page }) => {
    ...
    if ((await pskTab.count()) > 0) {
      ...
      // Look for AFC toggle
      const afcToggle = page.getByRole("checkbox", {
        name: /afc|auto.*frequency/i,
      });
...
test.describe("Decode - SSTV Decoder", () => {
  ...
  test("should support multiple SSTV modes (Martin, Scottie, Robot)", async ({
    page,
  }) => {
    await page.goto("/decode");
  ...
  test("should show progressive rendering with completion percentage", async ({
    page,
  }) => {
    await page.goto("/decode?mockSdr=1");
  ...
  test("should support save SSTV image", async ({ page }) => {
    ...
    const saveBtn = page.getByRole("button", {
      name: /save.*image|export.*image/i,
    });
...
test.describe("Analysis - Constellation Diagram", () => {
  test("should display constellation diagram", async ({ page }) => {
    ...
    const constellationCanvas = page.locator(
      'canvas[aria-label*="constellation" i]',
    );
...
test.describe("Analysis - Measurements", () => {
  ...
  test("should support export annotated images", async ({ page }) => {
    ...
    const exportBtn = page.getByRole("button", {
      name: /export.*image|save.*image/i,
    });
...
test.describe("Recordings - Navigation", () => {
  ...
  test("should support keyboard shortcut 5 for recordings", async ({
    page,
  }) => {
    await page.goto("/");
...
test.describe("Recordings - Library Display", () => {
  ...
  test("should display recording metadata (timestamp, duration, frequency)", async ({
    page,
  }) => {
    await page.goto("/recordings");
    ...
    const durationCol = page.getByRole("columnheader", {
      name: /duration|length/i,
    });
...
test.describe("Recordings - Filtering and Search", () => {
  test("should support filter by tags", async ({ page }) => {
    ...
    const filterControl = page.locator(
      '[aria-label*="filter" i], [placeholder*="filter" i]',
    );
...
test.describe("Recordings - Management", () => {
  ...
  test("should support tag editing", async ({ page }) => {
    ...
    const tagInput = page.locator(
      '[aria-label*="tag" i], input[placeholder*="tag" i]',
    );
  ...
  test("should warn when approaching storage quota", async ({ page }) => {
    ...
    const storageWarning = page.getByText(
      /storage.*full|quota.*exceeded|cleanup/i,
    );
