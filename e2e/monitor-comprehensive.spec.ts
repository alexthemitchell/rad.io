...
test.describe("Monitor - Core Functionality", () => {
  ...
  test("should display primary UI elements per UI Design Spec", async ({ page }) => {
...
test.describe("Monitor - Spectrum Visualization", () => {
  test("should render spectrum canvas with proper ARIA attributes", async ({ page }) => {
    await page.goto("/monitor?mockSdr=1");
    await startReception(page);
    
    // Find spectrum canvas
    const spectrumCanvas = page.locator('canvas[aria-label*="spectrum" i]').first();
...
test.describe("Monitor - Waterfall Display", () => {
  ...
  test("should scroll waterfall over time", async ({ page }) => {
    ...
    if (await waterfallToggle.count() > 0) {
      ...
      const waterfallCanvas = page.locator('canvas[aria-label*="waterfall" i]').first();
...
test.describe("Monitor - VFO and Frequency Control", () => {
  test("should display frequency in large, readable format", async ({ page }) => {
    await page.goto("/monitor?mockSdr=1");
...
test.describe("Monitor - Status Bar Metrics", () => {
  test("should display GPU rendering tier", async ({ page }) => {
    ...
    const tierText = statusBar.getByText(/WebGPU|WebGL2|WebGL|Worker|Canvas2D/i);
...
test.describe("Monitor - IQ Constellation Display", () => {
  test("should render IQ constellation diagram", async ({ page }) => {
    ...
    if (await iqCanvas.count() > 0) {
      ...
      const img = await iqCanvas.first().evaluate((c: HTMLCanvasElement) =>
        c.toDataURL()
      );
  ...
  test("should update IQ constellation in real-time", async ({ page }) => {
    ...
    if (await iqCanvas.count() > 0) {
      const img1 = await iqCanvas.first().evaluate((c: HTMLCanvasElement) =>
        c.toDataURL()
      );
      
      await page.waitForTimeout(300);
      
      const img2 = await iqCanvas.first().evaluate((c: HTMLCanvasElement) =>
        c.toDataURL()
      );
...
test.describe("Monitor - Mode Selection", () => {
  ...
  test("should support keyboard shortcut M to cycle modes", async ({ page }) => {
    await page.goto("/monitor?mockSdr=1");
